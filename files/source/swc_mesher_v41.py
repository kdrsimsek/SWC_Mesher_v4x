...
bl_info = {
    ...
    "version": (1, 0, 1),
    "blender": (4, 0, 0),  # Updated Blender version
...
class ExportCableModel_Operator(bpy.types.Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        if context.active_object is None:
            self.report({'ERROR'}, "Please select the cable model to export.")
            return {'CANCELLED'}
...
class MakeNeuronMetaPropGroup(bpy.types.PropertyGroup):
    ...
    def make_new_cable_model(self, context):
        ...
        mesh.attributes.new(name="index_number", type="FLOAT", domain='POINT')
        mesh.attributes.new(name="parent_index", type="FLOAT", domain='POINT')
        mesh.attributes.new(name="segment_type", type="FLOAT", domain='POINT')
        radius_attr = mesh.attributes.new(name="radius", type="FLOAT", domain='POINT')

        index_number_attr = mesh.attributes.get('index_number')
        parent_index_attr = mesh.attributes.get('parent_index')
        segment_type_attr = mesh.attributes.get('segment_type')
        radius_attr = mesh.attributes.get('radius')
        # Indexes
        index_number_attr.data[0].value = 1
        index_number_attr.data[1].value = 2

        # Parents
        parent_index_attr.data[0].value = -1
        parent_index_attr.data[1].value = 1

        # Radii
        radius_attr.data[0].value = -1.0
        radius_attr.data[1].value = -1.0

        # Segment type - just make it a dendrite
        segment_type_attr.data[0].value = 3
        segment_type_attr.data[1].value = 3
    ...
    def cable_model_add_func(self, context):
        ...
        if len(obj_list) > 0:
            for obj in obj_list:
                ...
                lrs = obj.data.attributes.keys()
                if not ('index_number' in lrs and 'parent_index' in lrs and 'segment_type' in lrs and 'radius' in lrs):
                    raise TypeError("Object: " + str(obj.name) + " is not a cable model (does not have the correct attributes). Select a different object, or re-import the cable from the SWC file.")
    ...
    def check_duplicate_verts(self, context):
        ...
        index_number_attr = attributes.get('index_number')
        n_v = len(ob.data.vertices)
        idx_vals = [index_number_attr.data[i].value for i in range(0, n_v)]
    ...
    def update_cable_model_post_edit(self, context):
        ...
        parent_index_attr = attributes.get('parent_index')
        ...
        i_start = 0
        index_number_attr.data[i_start].value = 1
        ...
        while len(verts_check) > 0:
            ...
            for i_conn in i_conns:
                # New index
                index_number_attr.data[i_conn].value = float(idx_assign)
                i_idx_dict[i_conn] = idx_assign
                idx_assign += 1

                # Get parent
                parent_index_attr.data[i_conn].value = float(idx_check)
        ...
        attributes = ob.data.attributes
        radius_attr = attributes.get('radius')
        n_v_new = len(i_idx_dict)
        for i_v in range(n_v_new, n_v):
            if i_v in index_number_attr.data:
                del index_number_attr.data[i_v]
            if i_v in parent_index_attr.data:
                del parent_index_attr.data[i_v]
            if i_v in radius_attr.data:
                del radius_attr.data[i_v]
    ...
    def make_spheres_from_object(self, context):
        ...
        for i_v, v in enumerate(ob.data.vertices):
            ...
            loc = v.co
            # Get the radius
            r = radius_attr.data[i_v].value
            if r < 0:
                r = self.new_sphere_radius
            # Get the id
            idx = int(index_number_attr.data[i_v].value)
    ...
    def update_cable_model_from_spheres(self, context):
        ...
        for i_v, v in enumerate(ob.data.vertices):
            ...
            radius_attr.data[i_v].value = new_r_dict[idx]
    ...
    def read_segments_from_object(self, context):
        ...
        index_number_attr = attributes.get('index_number')
        parent_index_attr = attributes.get('parent_index')
        segment_type_attr = attributes.get('segment_type')
        radius_attr = attributes.get('radius')
        ...
        i = 0
        for v in verts:
            n = int(index_number_attr.data[i].value)
            T = int(segment_type_attr.data[i].value)
            x = v.co.x
            y = v.co.y
            z = v.co.z
            R = radius_attr.data[i].value
            if R < 0:
                R = self.new_sphere_radius
            P = int(parent_index_attr.data[i].value)
    ...
    def get_swc_from_mesh_stick(self, context):
        ...
        # Index values
        n_idx = len(vs)
        id_value_list = [int(index_number_attr.data[i].value) for i in range(0, n_idx)]
        ...
        while idx < len(id_value_list) + 1:
            ...
            co = mat * vs[i_v].co
            radius_from_layer = radius_attr.data[i_v].value
            if radius_from_layer < 0:
                radius_from_layer = self.new_sphere_radius
            if idx == 1:
                lines.append("1 " + str(int(segment_type_attr.data[i_v].value)) + " " + str(co.x) + " " + str(co.y) + " " + str(co.z) + " " + str(radius_from_layer) + " -1")
            else:
                lines.append(
                    str(idx) + " " + str(int(segment_type_attr.data[i_v].value)) + " " + str(co.x) + " " + str(
                        co.y) + " " + str(co.z) + " " + str(radius_from_layer) + " " + str(
                        int(parent_index_attr.data[i_v].value)))
            idx += 1
    ...
    def build_neuron_stick_from_file(self, context):
        ...
        if (len(self.neuron_file_name) > 4 and self.neuron_file_name[-4:] == ".swc") or (len(self.neuron_file_name) > 8 and self.neuron_file_name[-8:] == ".swc.txt"):
            ...
            segment_type_attr = mesh.attributes.get('segment_type')
            radius_attr = mesh.attributes.get('radius')
            # print('index attr -> ', len(mesh.attributes.get('index_number').data))

            vert_index = 0
            # print(len(point_keys))
            for k in point_keys:
                p = point_dict[k]
                # print('vert_index -> ', vert_index)
                print("Adding metadata from " + str(p))
                index_number_attr.data[vert_index].value = int(p[0])
                parent_index_attr.data[vert_index].value = int(p[6])
                segment_type_attr.data[vert_index].value = int(p[1])
                radius_attr.data[vert_index].value = float(p[5])
                vert_index += 1
